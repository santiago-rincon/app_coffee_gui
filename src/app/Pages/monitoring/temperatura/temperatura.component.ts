...
export class TemperaturaComponent implements OnInit {
  ...
  yAxisLabel: string = 'Temperatura °C';
  ...
  constructor(
    ...
    this.extractInformation('Temperatura');
  ...
  extractInformation(collection: string) {
    this.firestore.getDataVariables(collection).subscribe((data) => {
      ...
      if (data.length != 0) {
        ...
        this.filterTemperature = this.dataTemperatura;
        let measures: number[] = [];
        for (const i of this.dataTemperatura) {
  ...
  consultDataForDate() {
    ...
    if (date === '') {
      ...
      const arrayFilter = this.filterTemperature.filter((element) => {
      ...
      if (arrayFilter.length == 0) {
        ...
        this.multiFilter = [{ name: 'Temperatura', series: [] }];
        for (const i of arrayFilter) {
  ...
  nodeSelected(e: any) {
    ...
    if (this.selectedNode == 0) {
      ...
      this.filterTemperature = [];
      this.filterTemperature = this.dataTemperatura.filter(
    ...
    this.filterTemperature.forEach((element) => {
      this.multi[0].series.push({
        name: element.time,
    ...
    if (this.filterTemperature.length != 0) {
      this.zeroData = false;
      let measures: number[] = [];
      this.filterTemperature.forEach((element) => {
